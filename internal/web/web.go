...
func (s *Servidor) Rutas() http.Handler {
	...
	mux.HandleFunc("/api/aprende", s.protegido(s.aprende))
	mux.HandleFunc("/api/glosario", s.protegido(s.glosario))
	mux.HandleFunc("/api/cebo", s.protegido(s.informeCebo))
	mux.HandleFunc("/api/sistema", s.protegido(s.estadoSistema))
	mux.HandleFunc("/api/iocs", s.protegido(s.iocs))
	mux.HandleFunc("/api/episodio/glosa", s.protegido(s.glosarEpisodio))
	mux.HandleFunc("/api/aprendizaje", s.protegido(s.aprendizaje))
	mux.HandleFunc("/api/proveedores", s.protegido(s.proveedoresIA))
	mux.HandleFunc("/api/artefacto/url-fondo", s.protegido(s.tocarURL))
	mux.HandleFunc("/api/artefacto/visto", s.protegido(s.artefactoVisto))
	mux.HandleFunc("/api/explicacion", s.protegido(s.explicacionEstado))
	...
	mux.HandleFunc("/api/radar", s.protegido(s.radar))
	mux.HandleFunc("/api/asistente", s.protegido(s.asistente))
	...
	mux.HandleFunc("/api/reportar-abuse", s.protegido(s.reportarAbuse))
	...
	mux.HandleFunc("/api/c2", s.protegido(s.infraC2))
	mux.HandleFunc("/api/botnets", s.protegido(s.botnets))
	mux.HandleFunc("/api/tuneles", s.protegido(s.tuneles))
